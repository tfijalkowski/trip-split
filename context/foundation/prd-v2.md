...
- Wydatki nie giną po odświeżeniu strony ani po ponownym otwarciu aplikacji — dane są trwale zapisane.
...
Logowanie: wyłącznie Google SSO. Każdy uczestnik musi posiadać konto Google — brak konta Google uniemożliwia dostęp do aplikacji.
...
1. **Czy organizator wyjazdu (twórca grupy) powinien mieć dodatkowe uprawnienia?** — np. zamknięcie rozliczenia, usuwanie cudzych wpisów. TBD po MVP. Owner: użytkownik. Block: nie (MVP działa z płaskim modelem uprawnień).
