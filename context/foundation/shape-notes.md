...
checkpoint:
  ...
  gray_areas_resolved:
    - topic: "rodzaj bólu"
      decision: "zbieranie kwot od wszystkich + ręczne liczenie kto komu ile winien"
    - topic: "persona"
      decision: "wszyscy uczestnicy równo — każdy może płacić za grupę"
    - topic: "insight"
      decision: "projekt edukacyjny (nauka budowania); gotowe narzędzia zbyt złożone / wymagają kont"
    - topic: "skala grupy"
      decision: "małe grupy 3–10 osób"
    - topic: "logowanie"
      decision: "Google SSO jedyna metoda; płaski model uprawnień dla MVP"
...
target_scale:
  users: small       # kilkanaście osób — projekt edukacyjny
  qps: low
  data_volume: small
timeline_budget:
  mvp_weeks: 3
...
Insight: istniejące narzędzia (Splitwise i podobne) są zbyt złożone dla okazjonalnych użytkowników — wymagają założenia kont przez wszystkich uczestników i oferują więcej funkcji niż potrzeba. Projekt ma też wymiar edukacyjny — celem jest nauka budowania aplikacji na rzeczywistym problemie.
...
- Wydatki są trwale zapisane po stronie serwera — nie giną po odświeżeniu strony.

## MVP Flow (locked)

```
1. Tomek zakłada konto
2. Tworzy grupę "Wakacje 2026"
3. Kopiuje link do grupy → wysyła przez WhatsApp
4. Uczestnicy klikają link, zakładają konto, dołączają do grupy
5. Każdy wprowadza wydatki (data / opis / kwota / kogo dotyczy / udział)
6. Każdy widzi listę wydatków + rozliczenie (kto komu ile winien)
```

`timeline_budget.mvp_weeks: 3` — zaproszenia emailowe przeniesione do v2 (scope-down decision).
...
Model uprawnień: w większości płaski — wszyscy uczestnicy mogą dodawać wydatki i przeglądać rozliczenie. Twórca grupy ma jedno dodatkowe uprawnienie: zamknięcie i otwarcie rozliczenia.

> Open question (resolved): twórca grupy otrzymuje uprawnienie zamknięcia/otwarcia rozliczenia (FR-015, FR-016). Delegowanie tego uprawnienia innym uczestnikom → v2.
